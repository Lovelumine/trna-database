...
export function FornaContainer(element, passedOptions = {}) {
        ...
        var mouseEventHelper = svg.append('svg:g')
            ...
            .on('mouseup', () => {
                if (mousedownNode && !linkContextMenuShown) {
                    dragLine.classed("fornac-transparent", true);
                }
                // clear mouse event vars
                resetMouseVars();
                //update()
            })
            .classed("fornac-mouseEventHelper", true)
    ...
    let toggleSelectNode = (node) => {
        ...
        visNodes.selectAll('g.gnode').filter((d) => { return node.uid == d.uid; })
            .classed("fornac-selectedNode", function(p) { return p.selected; });
    }
    ...
    let deselectAllNodes = () => {
        visNodes.selectAll('g.gnode')
            .classed("fornac-selectedNode", function(p) { return p.selected = false; });
    }
    ...
    self.brusher = d3.svg.brush()
        ...
        .on('brush', function() {
            ...
            visNodes.selectAll('g.gnode')
                .classed("fornac-selectedNode", function (d) {
    ...
    let createNewNodes = function(gnodesEnter) {
        ...
        nucleotideNodes.append('svg:path')
            .attr('class', "fornac-directionArrow")
            .attr('node_num', function(d) { return d.num; })

        // nucleotide outlines
        nucleotideNodes.append('svg:circle')
            .attr('class', "fornac-node")
            .attr('node_type', function(d) { return d.nodeType; })
            .attr('node_num', function(d) { return d.num; })
    ...
    function drawDirectionArrow(d) {
        ...
        let nodeStroke = parseFloat("1") / 2;
        let lengthMult = parseFloat("5");
        let arrowWidth = parseFloat("1");
    ...
    self.update = function () {
        ...
        gnodes.select('.' + "fornac-directionArrow").each(drawDirectionArrow);
    ...
    self.changeColorScheme = function(newColorScheme) {
        ...
        proteinNodes.classed("fornac-node", true)
            .attr('r', function(d) { return d.radius; });
    ...
    let updateStyle = () => {
        // Numbering
        visNodes.selectAll('[node_type=label]').classed("fornac-transparent", !self.displayParameters.displayNumbering);
        visNodes.selectAll('[label_type=label]').classed("fornac-transparent", !self.displayParameters.displayNumbering);
        visLinks.selectAll('[link_type=label_link]').classed("fornac-transparent", !self.displayParameters.displayNumbering);
        // Node Outline
        svg.selectAll('[node_type=nucleotide]').classed("fornac-transparent", !self.displayParameters.displayNodeOutline);
        // Node Labels
        visNodes.selectAll('[label_type=nucleotide]').classed("fornac-transparent", !self.displayParameters.displayNodeLabel);
        // Links
        svg.selectAll('[link_type=real],[link_type=basepair],[link_type=backbone],[link_type=pseudoknot],[link_type=protein_chain],[link_type=chain_chain],[link_type=external]').classed("fornac-transparent", !self.displayParameters.displayLinks);
        // Pseudoknot Links
        svg.selectAll('[link_type=pseudoknot]').classed("fornac-transparent", !self.displayParameters.displayPseudoknotLinks);
        // Protein Links
        svg.selectAll('[link_type=protein_chain]').classed("fornac-transparent", !self.displayParameters.displayProteinLinks);
        // Fake Links
        visLinks.selectAll('[link_type=fake]').classed("fornac-transparent", !self.options.displayAllLinks);
        visLinks.selectAll('[link_type=fake_fake]').classed("fornac-transparent", !self.options.displayAllLinks);
        // direction Arrows
        svg.selectAll('.' + "fornac-directionArrow").classed("fornac-transparent", !self.displayParameters.displayDirectionArrows);
    };
    ...
    let changeNode = function(nodeName, referenceNode) {
        ...
        uids.splice(newNodeNum-1, 1, Math.floor(Math.random() * 1000000));
    ...
    let insertNodeBeforeOrAfter = function(nodeName, referenceNode, positionOffset) {
        ...
        uids.splice(newNodeNum, 0, Math.floor(Math.random() * 1000000));
    ...
    let nodeMouseup = function(d,i) {
        ...
        if (mousedownNode) {
            ...
            let newLink = {source: mousedownNode, target: mouseupNode, linkType: 'basepair', value: 1, uid: Math.floor(Math.random() * 1000000)};
            ...
            if (newLink.source.rna != newLink.target.rna) {
                ...
                if ((newLink.source.num == 1 &&
                    ...
                    let linkMenu = [
                        {
                            ...
                            action: function(elm, d, i) {
                                ...
                                console.log('The data for this circle is: ' + d);
                                dragLine.attr('class', "fornac-transparent");
                    ...
                    linkContextMenu.apply(this, [d,i,true,
                        function() { dragLine.attr('class', "fornac-transparent") }]);
                } else {
    ...
    let nodeMousedown = function(d) {
        ...
        console.log(d)
        console.log(this)
        console.log("fornac-directionArrow")
        customData=d
    ...
    self.addExternalLinks = function(rnaJson, externalLinks) {
        ...
        for (let i = 0; i < externalLinks.length; i++) {
            let newLink = {linkType: 'external', value: 1, uid: Math.floor(Math.random() * 1000000),
